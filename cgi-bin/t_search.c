...
//gcc -o t_search.o t_search.c -lsqlite3 -ljansson  -lfcgi
...
int main() {
 while (FCGI_Accept() >= 0) {
    ...
    const char *sql = "SELECT * FROM teachers WHERE t_name LIKE ?";
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
            ...
            json_object_set_new(object, "t_id", json_string((const char*)sqlite3_column_text(stmt, 0)));
            json_object_set_new(object, "t_name", json_string((const char*)sqlite3_column_text(stmt, 1)));
            json_object_set_new(object, "t_sex", json_string((const char*)sqlite3_column_text(stmt, 2)));
            json_object_set_new(object, "t_contact", json_string((const char*)sqlite3_column_text(stmt, 3)));
            json_object_set_new(object, "t_address", json_string((const char*)sqlite3_column_text(stmt, 4)));
            json_object_set_new(object, "t_dept", json_string((const char*)sqlite3_column_text(stmt, 5)));
...
//curl -X POST -H "Content-Type: application/json" -d '[{"name": "name", "value": "bo"}]' http://172.28.98.30:8080/test/test/cgi-bin/t_search.o
